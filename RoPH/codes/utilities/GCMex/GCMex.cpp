    ...
    )
{
  ...
  enum {IN_CLASS=0,IN_UNARY,IN_PAIRWISE,IN_LABELCOST,IN_EXPANSION,IN_MAXITER} ;//0,1,2,3,4
  enum {OUT_LABELS=0, OUT_ENERGY, OUT_ENERGYAFTER} ;//0,1,2
   ...
   ***************************************************************************/
  if (nin != 5 && nin != 6)
    mexErrMsgTxt("Five or six arguments are required.");
  if (nin == 6)
	max_iter = *mxGetPr(in[IN_MAXITER]);
  if (nout > 3)
    mexErrMsgTxt("At most three outputs are allowed.");

  if(mxGetClassID(in[IN_CLASS]) != mxDOUBLE_CLASS)//初始类别必须是double类型
    mexErrMsgTxt("Class must be a vector of class DOUBLE");
  if(mxGetM(in[IN_CLASS]) != 1 && mxGetN(in[IN_CLASS]) != 1)//向量
    mexErrMsgTxt("Class must be a vector");

  if(mxGetNumberOfDimensions(in[IN_UNARY]) != 2 ||//一元项是单精度矩阵
     mxGetClassID(in[IN_UNARY]) != mxSINGLE_CLASS)
    mexErrMsgTxt("Unary term must be a matrix of class SINGLE");

  if(mxGetClassID(in[IN_LABELCOST]) != mxSINGLE_CLASS)//标签损失是单精度矩阵
    mexErrMsgTxt("Labelcost term must be a matrix of class SINGLE");

  expansion = *mxGetPr(in[IN_EXPANSION]) > 0;
  int num_labels = mxGetM(in[IN_UNARY]);//标签数
  int num_pixels = mxGetN(in[IN_UNARY]);//像素数

  if(mxGetM(in[IN_CLASS]) != num_pixels && mxGetN(in[IN_CLASS]) != num_pixels)//IN_CLASS可以是行向量也可以是列向量
  ...
  mwSize dims[2] = {1,0};
  out[OUT_ENERGY] = mxCreateNumericArray(1, dims, mxDOUBLE_CLASS, mxREAL);//使用Array可以创建多维矩阵
  ...
  double * labels = mxGetPr(out[OUT_LABELS]);//输出标签
